...
if seleccion == "Resumen General":
    ...
    <div style="display: flex; gap: 20px; margin-bottom: 20px; flex-wrap: wrap;">
        <div style="background: linear-gradient(135deg, #1f3044 0%, #16222a 100%); padding: 12px 20px; border-radius: 8px; border: 1px solid #00d2ff; box-shadow: 0 4px 10px rgba(0,210,255,0.2);">
            <span style="color: #b0b3b8; font-size: 12px; font-weight: bold; text-transform: uppercase; display: block;">TOTAL ATENCIONES ({seleccion.upper()})</span>
        ...
        <div style="background: linear-gradient(135deg, #1f3044 0%, #16222a 100%); padding: 12px 20px; border-radius: 8px; border: 1px solid #31333f; box-shadow: 0 4px 10px rgba(0,0,0,0.2);">
            <span style="color: #b0b3b8; font-size: 12px; font-weight: bold; text-transform: uppercase; display: block;">FECHA DE ACTUALIZACIÓN</span>
            <span style="color: #34d399; font-size: 18px; font-weight: 800;">{fecha_actualizacion_jornada}</span>
        </div>
    </div>
    """,
        unsafe_allow_html=True,
    ...
    with tab_v1:
        if os.path.exists(archivo_esp):
            ...
            st.components.v1.html(
                renderizar_tabla_html_pro(df_esp_v), height=380, scrolling=True
            )
        else:
            st.info(f"No hay registros de especialidades para {seleccion}.")

    with tab_v2:
        if os.path.exists(archivo_apo):
            df_apo_v = cargar_datos_cache(archivo_apo)
            st.components.v1.html(
                renderizar_tabla_html_pro(df_apo_v), height=380, scrolling=True
            )
        else:
            st.info(f"No hay registros de apoyo social para {seleccion}.")

    with tab_v3:
        if os.path.exists(archivo_demo):
            df_demo_v = cargar_datos_cache(archivo_demo)
            st.components.v1.html(
                renderizar_tabla_html_pro(df_demo_v), height=250, scrolling=False
            )
        else:
            st.info(f"No hay registros demográficos para {seleccion}.")

    col_dl1, col_dl2 = st.columns([2, 8])
    with col_dl1:
        df_descarga_actual = (
            cargar_datos_cache(archivo_esp)
            if os.path.exists(archivo_esp)
            else pd.DataFrame()
        )
        st.download_button(
            "📥 Descargar Reporte en Excel",
            data=convertir_df_a_excel(df_descarga_actual),
    ...
    if os.path.exists(archivo_cat):
        ...
        if not df_cat_vista.empty:
            if "ATENCIONES" in df_cat_vista.columns:
                ...
                val_total = int(df_cat_vista["ATENCIONES_NUM"].sum())
                cols_limpias = [
                    c for c in df_cat_vista.columns if c != "ATENCIONES_NUM"
                ]
            else:
                val_total = len(df_cat_vista)
                cols_limpias = list(df_cat_vista.columns)

            st.markdown(
                f"""
               <div class="total-tab">
                   <span style="color: #b0b3b8; font-size: 12px; font-weight: bold; text-transform: uppercase;">TOTAL REGISTROS {seleccion.upper()}: </span>
                   <span style="color: #ffffff; font-size: 20px; font-weight: 900; margin-left: 10px;">{formatear_numero(val_total)}</span>
               </div>
               """,
                unsafe_allow_html=True,
            )
