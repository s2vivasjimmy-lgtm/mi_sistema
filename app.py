...
with st.sidebar:
    st.header("📋 Registros")
    if not st.session_state.admin_logueado and not st.session_state.hosp_logueado:
        with st.expander("🔐 Acceso Hospitales"):
            user = st.text_input("Usuario")
            pwd = st.text_input("Contraseña", type="password")
            if st.button("Ingresar"):
                if user == "Admin" and pwd == "diges12..": st.session_state.admin_logueado = True; st.rerun()
                elif user in HOSPITALES_CONFIG and pwd == HOSPITALES_CONFIG[user]["pwd"]:
                    st.session_state.hosp_logueado = True; st.session_state.hosp_id = user; st.rerun()
    ...
    if (st.session_state.admin_logueado or st.session_state.hosp_logueado) and st.button("❌ Cerrar Sesión"):
        st.session_state.admin_logueado = False; st.session_state.hosp_logueado = False; st.rerun()
...
if st.session_state.hosp_logueado:
    ...
    val = st.number_input("Carga de Atenciones Diarias:", value=int(pd.read_csv(archivo_h)['ATENCIONES'].iloc[0]))
    if st.button("💾 Guardar y Actualizar"):
        pd.DataFrame({'ATENCIONES': [val]}).to_csv(archivo_h, index=False)
        df_maestro = pd.read_csv(ARCHIVO_RESUMEN)
        suma = sum(int(pd.read_csv(f"{hid}.csv")['ATENCIONES'].iloc[0]) for hid, info in HOSPITALES_CONFIG.items() if info['tipo'] == hosp['tipo'] and os.path.exists(f"{hid}.csv"))
        df_maestro.at[0, hosp['tipo']] = suma
        df_maestro.to_csv(ARCHIVO_RESUMEN, index=False)
        guardar_en_github(ARCHIVO_RESUMEN)
        st.success("Guardado.")

elif st.session_state.admin_logueado:
    st.header(f"📝 Edición Admin: {seleccion}")
    ...
    df_e = st.data_editor(pd.read_csv(archivo_a_editar, dtype=str), use_container_width=True, num_rows="dynamic")
    if st.button("💾 Guardar Cambios"):
        df_e.to_csv(archivo_a_editar, index=False); guardar_en_github(archivo_a_editar); st.rerun()

else:
    # --- VISUALIZACIÓN PÚBLICA INTEGRAL ---
    ...
    if seleccion == "Resumen General":
        ...
        if os.path.exists(archivo_detalle):
            ...
            if "NACIONALIAD" in df_detalle.columns and "ATENCIONES" in df_detalle.columns:
                ...
                sn, se = resumen.get('NACIONAL', 0), resumen.get('EXTRANJERO', 0) + resumen.get('ESTRANJERO', 0)
                cols = st.columns(2)
                cols[0].metric("Total Nacionales", f"{int(sn):,}".replace(",", "."))
                cols[1].metric("Total Extranjeros", f"{int(se):,}".replace(",", "."))
                fig = go.Figure(data=[go.Pie(labels=['NACIONAL', 'EXTRANJERO'], values=[sn, se], hole=.6, marker_colors=['#FF0000', '#002060'])])
                st.plotly_chart(fig, use_container_width=True)
            st.dataframe(df_detalle, use_container_width=True)
            excel_data = convertir_df_a_excel(df_detalle)
            st.download_button("📥 Descargar Reporte en Excel", data=excel_data, file_name=f"{seleccion}.xlsx")
        else:
